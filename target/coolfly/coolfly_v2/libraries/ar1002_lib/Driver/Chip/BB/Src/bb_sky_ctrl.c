...
static void process_lock_state()
{
	...
	if (stru_skystatus.check_lock_times > 0)
	{
		...
		stru_skystatus.rst_start_time = HAL_GetSysMsTick();
		sky_soft_reset();
		DLOG_Warning("rst time: %ld", stru_skystatus.rst_start_time);
	}
...
skip_sky_select:
    ...
    DLOG_Warning("req vtskip %d,time %ld,cvt:%d -> vt:%d,", get, SysTicks_GetTickCount(), context.cur_IT_ch, ch);
}
...
static void sky_handle_MCS_cmd(void)
{
    ...
    if(context.mcs_switch_state == 0 && context.qam_ldpc != data0)
    {
        ...
        if(val == 0)
        {
            ...
            DLOG_Info("%d r206=%x, %ld",drop_usb0, BB_ReadReg(PAGE2, 0x06),time);
    ...
    else if(context.mcs_switch_state == 2)
    {
        ...
        if(flag)
        {
            ...
            DLOG_Info("%x delta %ld ms",BB_ReadReg(PAGE2, 0x06),SysTicks_GetDiff(time,SysTicks_GetTickCount()));
    ...
    if(class == WIRELESS_FREQ_CHANGE)
    {
        switch(item)
        {
            ...
            case IT_CHANNEL_SELECT:
            {
				...
				DLOG_Critical("it channel value %lx\r\n", value);
                break;
            }
...
static void sky_vtSkip_process(void)
{
    ...
    {
        debug_cnt = 0;
        DLOG_Warning("nlbt enable, %ld",swp_e);
    }
