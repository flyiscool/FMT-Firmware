...
int  __attribute__ ((section(".h264"))) BB_add_cmds(uint8_t type, uint32_t param0, uint32_t param1, uint32_t param2, uint32_t param3)
{
    ...
    DLOG_Warning("type:%d:%ld %ld %ld %ld", type, param0, param1, param2, param3);
    switch(type)
    {
#if 0
        case 0:
        ...
        case 26:
        {
            DLOG_Warning("rc rate:%ld",BB_GetRcRate(param0));
            ret = 0;
            break;
        }
#if 0  
        ...
        case 38:
        {
			...
			DLOG_Warning("non-lbt :  %ld", param0);
...
static void __attribute__ ((section(".h264"))) BB_HandleEventsCallback(void *p)
{
    ...
    else
    {
		...
		else if((class == WIRELESS_OTHER) && (item == WIRELESS_RF_PA_MODE))
		{
				...
				DLOG_Critical("is_manul=%ld,set rc patten %d:%d:%d:%d:%d",value,msg[0],msg[1],msg[2],msg[3],msg[4]); 
		}
        ...
        volatile int ret = BB_InsertCmd(0, (STRU_WIRELESS_CONFIG_CHANGE * )p);
    }
}
...
void BB_GenHashId(uint8_t *rcid, uint8_t *vtid,ENUM_BB_MODE en_mode)
{
    ...
    if(en_mode == BB_SKY_MODE)
    {
        crc = crc16((char *)id,RC_ID_SIZE+VT_ID_SIZE);
    ...
    {
        crc = crc32((char *)id,RC_ID_SIZE+VT_ID_SIZE);
